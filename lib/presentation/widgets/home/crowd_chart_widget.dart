...
class CrowdChartWidget extends StatefulWidget {
  const CrowdChartWidget({super.key});

  @override
  State<CrowdChartWidget> createState() => _CrowdChartWidgetState();
}

class _CrowdChartWidgetState extends State<CrowdChartWidget> {
  ...
  Future<void> _loadCrowdData() async {
    try {
      final data = await CrowdRepository.getHourlyCrowdAggregated();
  ...
  void _listenToRealtime() {
    CrowdRepository.streamTodayCrowdData().listen((snapshot) {
      ...
      for (final doc in snapshot.docs) {
        ...
        final count = data['crowdCount'] as int;
      ...
      final maxVal = updated.values.isEmpty
          ? 1
          : updated.values.reduce((a, b) => a > b ? a : b);
  ...
  double _getBarHeight(int hour) {
    if (_hourlyData.isEmpty || (_hourlyData[hour] ?? 0) == 0) {
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          const Text(
            'Plan your visit to avoid the crowds.',
              ...
              children: [
                ...
                Column(
                  ...
                  children: [
                    ...
                    Text('${(_maxCount * 0.75).toInt()}',
                        style: const TextStyle(
                            color: Colors.white70, fontSize: 8)),
                    const SizedBox(height: 10),
                    Text('${(_maxCount * 0.5).toInt()}',
                        style: const TextStyle(
                            color: Colors.white70, fontSize: 8)),
                    const SizedBox(height: 10),
                    Text('${(_maxCount * 0.25).toInt()}',
                        style: const TextStyle(
                            color: Colors.white70, fontSize: 8)),
                  ],
                ),
